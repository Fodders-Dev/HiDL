...
## Deploy (Railway, SQLite + Volume)
1. Создай Volume и замаунть в сервис в `/data`.
2. Env vars:
   - `BOT_TOKEN` — токен бота
   - `DATABASE_URL=/data/hidl.db`
   - (опц.) `DEBUG_LOG=1`
3. Если нужно перенести локальную базу 1 раз (чтобы не терять пользователей/историю):
   - Залей `hidl.db` в публичное место (временная ссылка)
   - Поставь `BOOTSTRAP_DB_URL=<ссылка-на-hidl.db>` и сделай redeploy
   - После первого успешного старта убери `BOOTSTRAP_DB_URL`
...
- Спорт: разминка/домашние тренировки, прогулки, фокус 20/10 с кнопкой стоп, учёт веса/цели.
