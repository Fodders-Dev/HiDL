...
async def cmd_start(message: types.Message, state: FSMContext, db) -> None:
    ...
    await message.answer(
        ...
        "Будем больше работать через кнопки, команды помнить не нужно.\n\n"
...
async def reg_sleep(message: types.Message, state: FSMContext, db) -> None:
    ...
    await message.answer(
        ...
        "Главное меню: 📅 Сегодня, 🍽 Еда, 🧹 Дом, 💰 Деньги, 🚶 Движение, ⚙ Настройки.\n"
        "Все доступно по кнопкам ниже. Если запутаешься — /help.",
        reply_markup=main_menu_keyboard(),
    )
