        ...
        buttons.append(
            [
                InlineKeyboardButton(
                    text=f"🗑 {row.get('title','Напоминание')} ({row.get('reminder_time','--:--')}, каждые {row.get('frequency_days',1)} д{wd})",
...
async def add_reminder_start(message: types.Message, state: FSMContext, db) -> None:
    ...
    await message.answer("Что напомнить? Введи короткий текст." + pause_note, reply_markup=main_menu_keyboard())
...
async def reminder_when(callback: types.CallbackQuery, state: FSMContext, db) -> None:
    ...
    if choice == "plus1":
        ...
        await callback.message.answer(
            f"Напоминание добавлено: {data['title']} в {hhmm} (ежедневно).",
...
async def add_reminder_time(message: types.Message, state: FSMContext, db) -> None:
    ...
    if abs_time:
        ...
        if not parse_hhmm(hhmm_norm):
            await message.answer("Не распознал время. Формат HH:MM или «через 2 часа».")
            return
...
async def _save_reminder_with_frequency(
    ...
    await message.answer(
        f"Напоминание добавлено: {data.get('title','Напоминание')} в {reminder_time} каждые {freq} д.{note}",
        reply_markup=main_menu_keyboard(),
    )
