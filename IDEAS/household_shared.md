...
- Уже реализовано:
  - личная кладовка продуктов (`pantry_items`) на пользователя;
  - отдельная таблица `supplies` для бытовой химии, которая может стать основой общего инвентаря;
  - регулярные дела по дому (regular_tasks) с зонами и очками.
- Next sprint candidates:
  - полноценная модель `households` + `household_members`;
  - общий pantry и supplies для всех участников дома;
  - мягкий мастер «мы живём вместе» в настройках.
